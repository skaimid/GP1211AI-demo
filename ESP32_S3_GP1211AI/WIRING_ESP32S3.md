...
| ESP32-S3 GPIO | 设备 | 功能描述 | 备注 |
|---------------|------|---------|------|
| **VFD 显示屏** |
| GPIO10 | VFD BK | 亮度控制PWM | LEDC 通道 0 |
| GPIO11 | VFD LAT | 数据锁存 | 普通 GPIO |
| GPIO12 | VFD CLKG | 栅极时钟 | 普通 GPIO |
| GPIO13 | VFD SIG | 信号控制 | 普通 GPIO |
| GPIO14 | VFD SIA/MOSI | SPI 数据输出 | SPI MOSI |
| GPIO15 | VFD CLKA/SCK | SPI 时钟 | SPI SCK |
| GPIO16 | VFD HV_EN | 高压使能 | 普通 GPIO |
| GPIO17 | VFD FL_EN | 灯丝使能 | 普通 GPIO |
| **AHT20 温湿度传感器** |
| GPIO8 | AHT20 SDA | I2C 数据线 | 支持 3.3V |
| GPIO9 | AHT20 SCL | I2C 时钟线 | 支持 3.3V |
| 3.3V | AHT20 VCC | 传感器电源 | 3.3V 供电 |
| GND | AHT20 GND | 传感器地 | 公共地 |
| **按键（可选）** |
| GPIO4 | K_U | 增加亮度按键 | 内部上拉 |
| GPIO5 | K_D | 减少亮度按键 | 内部上拉 |
| GPIO6 | K_M | 菜单按键 | 内部上拉 |
| **电源** |
| 3.3V | - | 电源正极 | AHT20 使用 3.3V |
| 5V | - | VFD 电源 | 如 VFD 需要 5V |
| GND | - | 电源负极 | 所有地必须共地 |
...
### 第八步：AHT20 温湿度传感器（I2C）

⚠️ **AHT20 是 3.3V 设备，直接连接到 ESP32-S3，无需电平转换！**

```
ESP32-S3 3.3V ----> AHT20 VCC
ESP32-S3 GND  ----> AHT20 GND
ESP32-S3 GPIO8 ---> AHT20 SDA (I2C 数据线)
ESP32-S3 GPIO9 ---> AHT20 SCL (I2C 时钟线)

注意：
1. AHT20 内部已有上拉电阻，无需外接
2. 如果 I2C 总线较长（>30cm），建议添加 4.7kΩ 上拉电阻
3. SDA/SCL 连接线应尽量短，避免干扰
```
...
```
         AHT20 模块
      ┌──────────────┐
VCC ──┤ VCC      SDA ├── GPIO8 (ESP32-S3)
GND ──┤ GND      SCL ├── GPIO9 (ESP32-S3)
      └──────────────┘
       ↑         ↑
       |         |
    3.3V      I2C 总线
...
### 方案：ESP32-S3 + VFD (通过 TXS0108E) + AHT20
...
GND ───────────────┤ GND     │─┬──┴───────────────┤ GND      │                   │          │
                   │         │ │                  │          │                   │          │
GPIO14 (MOSI) ─────┤         │────────────────────┤ A1    B1 ├───────────────────┤ SIA      │
GPIO15 (SCK)  ─────┤         │────────────────────┤ A2    B2 ├───────────────────┤ CLKA     │
GPIO11 (LAT)  ─────┤         │────────────────────┤ A3    B3 ├───────────────────┤ LAT      │
GPIO12 (CLKG) ─────┤         │────────────────────┤ A4    B4 ├───────────────────┤ CLKG     │
GPIO13 (SIG)  ─────┤         │────────────────────┤ A5    B5 ├───────────────────┤ SIG      │
GPIO10 (BK)   ─────┤         │────────────────────┤ A6    B6 ├───────────────────┤ BK       │
...
GPIO9 (SCL)   ─────┤         │      │  (接GND/悬空) │ VCCB     ├────────────────────┤          │
                   ...
                   └─────────┘      │                                             └──────────┘
                        ...
                        └─────────────┐        │
                          ...
                          3.3V ──┤ VCC     │   │
                           GND ──┤ GND     │   │
                      GPIO8 (SDA)┤ SDA     │───┘ (I2C 总线)
                      GPIO9 (SCL)┤ SCL     │
                                 └─────────┘
                                  AHT20 模块

注意：
1. AHT20 直接连接 ESP32-S3 (3.3V)，无需电平转换
2. VFD 信号通过 TXS0108E 进行 3.3V → 5V 电平转换
3. 所有 GND 必须共地
4. 按键和 I2C 分别使用不同的 GPIO
```
