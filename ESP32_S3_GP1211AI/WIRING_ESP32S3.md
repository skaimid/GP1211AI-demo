...
|---------------|------|---------|------|
| **VFD 显示屏** |
| GPIO12 | VFD BK | 亮度控制PWM | LEDC 通道 0 |
| GPIO13 | VFD LAT | 数据锁存 | 普通 GPIO |
| GPIO14 | VFD CLKG | 栅极时钟 | 普通 GPIO |
| GPIO15 | VFD SIG | 信号控制 | 普通 GPIO |
| GPIO11 | VFD SIA/MOSI | SPI 数据输出 | HSPI MOSI |
| GPIO12 | VFD CLKA/SCK | SPI 时钟 | HSPI SCK |
| GPIO16 | VFD HV_EN | 高压使能 | 普通 GPIO |
| GPIO17 | VFD FL_EN | 灯丝使能 | 普通 GPIO |
| **AHT20 温湿度传感器** |
| GPIO21 | AHT20 SDA | I2C 数据线 | 支持 3.3V |
| GPIO22 | AHT20 SCL | I2C 时钟线 | 支持 3.3V |
...
ESP32-S3 GPIO21 ---> AHT20 SDA (I2C 数据线)
ESP32-S3 GPIO22 ---> AHT20 SCL (I2C 时钟线)
...
```
         AHT20 模块
      ┌──────────────┐
VCC ──┤ VCC      SDA ├── GPIO21 (ESP32-S3)
GND ──┤ GND      SCL ├── GPIO22 (ESP32-S3)
      └──────────────┘
       ↑         ↑
       |         |
    3.3V      I2C 总线
...
GPIO20 (K_M)  ─────┤         │    └─┬─┘           │          │                   │          │
                   │         │      │              │          │                   │          │
GPIO21 (SDA)  ─────┤         │──────│──────────────┤ OE       │            5V ────┤ VCC      │
GPIO22 (SCL)  ─────┤         │      │  (接GND/悬空) │ VCCB     ├────────────────────┤          │
                   │         │      │              └──────────┘            GND ───┤ GND      │
                   └─────────┘      │                                             └──────────┘
                        │ │         │
                        │ └─────────┴──────────┐
